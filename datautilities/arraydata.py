...
class InputData(object):
    ...
    def set_scalars(self, data):
        ...
        if hasattr(data.network.violation_cost, "e_vio_cost"):
            self.c_e = float(data.network.violation_cost.e_vio_cost)
        else:
            # TODO: read this from data, validate, including preferred value
            # in config, check value > 0, add assumptions to formulation,
            # add symbol and algebra to formulation and sandbox dataset
            self.c_e = 1.0e6 # 1.0e4 USD/MWh = 1.0e6 USD/pu-h
